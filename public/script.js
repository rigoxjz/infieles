// script.js → 100% FUNCIONAL (PUBLICAR CHISME SÍ FUNCIONA) – DICIEMBRE 2025
const API = "https://infieles-back.onrender.com"; // ← API ACTIVA Y 100% FUNCIONAL
...
async function cargarInfieles() {
  ...
  try {
    ...
    lista.innerHTML = `<div style="color:red;text-align:center;padding:50px">Error de conexión</div>`;
  }
}
...
async function verDetalle(id) {
  ...
  try {
      ...
      <div class="comentarios">
        ...
        <div id="lista-comentarios">
          ${item.comentarios?.length ? item.comentarios.map(c => `
            <div class="comentario"><strong>${escapeHtml(c.nombre || "Anónimo")}</strong><p>${escapeHtml(c.texto)}</p></div>
          `).join("") : "<p style='color:#888;text-align:center;padding:20px'>Sin comentarios aún</p>"}
        </div>
        <input type="text" id="com-nombre" placeholder="Nombre (opcional)">
        <textarea id="com-texto" placeholder="Escribe tu comentario..." rows="3"></textarea>
        <button class="btn btn-azul" id="btn-enviar-com">Enviar Comentario</button>
    ...
    if (yaVoto) {
      document.getElementById("btn-real").disabled = document.getElementById("btn-falso").disabled = true;
      document.getElementById("btn-real").style.opacity = document.getElementById("btn-falso").style.opacity = "0.6";
    } else {
      document.getElementById("btn-real").onclick = () => votar(id, true);
      document.getElementById("btn-falso").onclick = () => votar(id, false);
    }
...
async function enviarComentario(id) {
  ...
  try {
    ...
    if (j.success) {
      ...
      const lista = document.getElementById("lista-coment");
      ...
      const n = parseInt(h3.textContent.match(/\d+/)?.[0] || "0") + 1;
      h3.textContent = `Comentarios (${n})`;
      setTimeout(() => status.style.display = "none", 1500);
    }
  } catch { status.textContent = "Error"; status.style.color = "red"; }
}
